...
| - | - |
| `{{internal.logins}}` | Substituted with "allowed logins" parameter used in `tctl users add [user] <allowed logins>` command. This applies only to users stored in Teleport's local database. |
...
## Role-Based Access Control for hosts
...
`allow`/`deny` labels in a role definition conforming to Role-Based Access Control (RBAC) best-practices.
...
with allow rule for the label `'environment': 'staging'`.
