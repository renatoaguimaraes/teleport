...
- `u2f` implements the [U2F](https://en.wikipedia.org/wiki/Universal\_2nd_Factor) standard for utilizing hardware (USB) keys for 2nd-factor. You can use [YubiKeys](https://www.yubico.com/),
...
- `off` turns off 2nd-factor authentication.
...
credentials are correct, the auth server generates and signs a new certificate, and the client stores this key and will use it for subsequent logins. The key
...
Teleport Node Tunneling lets you add a remote node to an existing Teleport Cluster via a tunnel.
...
1. `static labels` do not change over time, while the [`teleport`](cli-docs.mdx#teleport)
   process is running.  Examples of static labels are the physical location of nodes,
   the name of the environment (staging vs production), etc.
2. `dynamic labels` also known as "label commands" allow to generate labels at runtime. Teleport will execute an external command on a node at a configurable frequency and the output of the command becomes the label value. Examples include reporting load averages, presence of a process, time since the last reboot, etc.
...
Architecture](architecture/authentication.mdx#audit-log) to learn more about how audit logs and session recordings are designed.
...
records the entire stream of bytes going to/from standard input and the standard
...
| role | A role assumed by users. The open-source Teleport only includes one role: "admin", but Enterprise teleport users can define their roles. |
...
to have a direct connection to other clusters' proxy servers. Trusted clusters can also define restrictions on user access.
...
- Use a load balancer to create a single auth API access point (AP) and specify this AP in the `auth_servers` section of Teleport configuration for all nodes in a cluster. This load balancer should do TCP level forwarding.
...
- Install etcd and configure peer and client TLS authentication using the [etcd
  ...
  - You can use [this script provided by etcd](https://github.com/etcd-io/etcd/tree/master/hack/tls-setup) if you
...
- Make sure you have an AWS access key and a secret key that give you access to
...
`data_dir`. If a license file path is not set, Teleport will look for the
